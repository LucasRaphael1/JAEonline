...
abstract class Model
{
     ...
     * @return Model
     */
    public function query(?string $query, ?string $params = null): Model
    {
        if ($params) {
            $this->query = $query;
            parse_str($params, $this->params);
     ...
     */
    public function fetch(bool $all = false, bool $array = false)
    {
        try {
     ...
     * @return bool
     */
    public function softDestroy(): bool
    {
        if (empty($this->id)) {
            return false;
        }
    ...
    public function lessDeleted(): self
    {
        $has = mb_strpos($this->query, 'WHERE');
        if ($has === false) {
            $this->query .= ' WHERE deleted_at IS NULL ';
        } else {
            $data = explode('WHERE', $this->query);
            $this->query = $data[0] . ' WHERE deleted_at IS NULL AND ' . $data[1];
        }
        return $this;
    }
